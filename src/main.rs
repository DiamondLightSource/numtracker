...
use cli::{Cli, Command, ConfigAction, ServeOptions, TemplateAction, TemplateKind};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    match args.command {
        ...
        Command::Config(opts) => configure(&args.db, opts.action).await,
    }
    Ok(())
}
...
async fn configure(db: &Path, opts: ConfigAction) {
    let db = SqliteScanPathService::connect(db).await.unwrap();
    match opts {
