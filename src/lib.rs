...
#[derive(Clone)]
pub struct SplitBackend<Num, PT> {
    numbers: Num,
    paths: PT,
}

#[derive(Clone)]
pub struct CombinedBackend<B>(B);

impl<B> CombinedBackend<B> {
    pub fn new(backend: B) -> Self {
        Self(backend)
    }
}

impl<Num, PT> SplitBackend<Num, PT> {
    pub fn new(numbers: Num, paths: PT) -> Self {
        Self { numbers, paths }
    }
}

impl<Num: ScanNumberBackend, PT: Clone + Sync + Send> ScanNumberBackend for SplitBackend<Num, PT> {
    type NumberError = Num::NumberError;

    fn next_scan_number(
        &self,
        beamline: &str,
    ) -> impl Future<Output = Result<usize, Self::NumberError>> + Send {
        self.numbers.next_scan_number(beamline)
    }
}

impl<B: ScanNumberBackend> ScanNumberBackend for CombinedBackend<B> {
    type NumberError = B::NumberError;

    fn next_scan_number(
        &self,
        beamline: &str,
    ) -> impl Future<Output = Result<usize, Self::NumberError>> + Send {
        self.0.next_scan_number(beamline)
    }
}

impl<Num: Clone + Sync + Send, PT: PathTemplateBackend> PathTemplateBackend
    for SplitBackend<Num, PT>
{
    type TemplateErr = PT::TemplateErr;

    fn visit_directory_template(
        &self,
        beamline: &str,
    ) -> impl Future<Output = Result<VisitTemplate, Self::TemplateErr>> + Send {
        self.paths.visit_directory_template(beamline)
    }
    ...
    fn scan_file_template(
        ...
        self.paths.scan_file_template(beamline)
    }
    ...
    fn detector_file_template(
        ...
        self.paths.detector_file_template(bl)
    }
}

impl<C: PathTemplateBackend> PathTemplateBackend for CombinedBackend<C> {
    type TemplateErr = C::TemplateErr;

    fn visit_directory_template(
        &self,
        beamline: &str,
    ) -> impl Future<Output = Result<VisitTemplate, Self::TemplateErr>> + Send {
        self.0.visit_directory_template(beamline)
    }
    ...
    fn scan_file_template(
        ...
        self.0.scan_file_template(beamline)
    }
    ...
    fn detector_file_template(
        ...
        self.0.detector_file_template(bl)
    }
}
